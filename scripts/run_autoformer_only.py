...
AUTOFORMER_PATH = os.path.join(ROOT_DIR, 'baseline_models', 'Autoformer')
sys.path.insert(0, AUTOFORMER_PATH)
os.chdir(AUTOFORMER_PATH)
...
def main():
    ...
    for seed in seeds:
        ...
        try:
            model = AutoformerWrapper(AutoformerConfig(96, 96, 19)).to(device)
    ...
    all_results = [{'model': 'Autoformer', 'asset': asset, 'MAE_mean': np.mean(seed_results[asset]['MAE']),
    ...
    if all_results:
        ...
        print(f"\nAutoformer appended. Average: MAE={all_results[-1]['MAE_mean']:.4f}±{all_results[-1]['MAE_std']:.4f}")
